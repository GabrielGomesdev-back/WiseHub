...
public class CrudService {
    ...
    public ResponseEntity<ObjectNode> criarUsuario(CriarUsuarioDTO json) {
        try {
            ...
            mailService.enviarEmailCriacaoUsuario(usuario.getEmail(), "Criação de conta na wiseHub", "Olá Hooper, "+usuario.getNome()+" Seja bem vindo(a) aos serviços WiseHub agora que sua conta foi criada você pode ter acesso as informações sobre o seu time e seu jogador favorito da NBA sempre que quiser !!!\n");
    ...
    public ResponseEntity<ObjectNode> atualizarUsuario(AtualizarUsuarioDTO json) {
        try {
            ...
            usuario = usuarioRepository.save(usuario);
            mailService.enviarEmailCriacaoUsuario(usuario.getEmail(), "Atualização de conta na wiseHub", "Olá Hooper, "+usuario.getNome()+" Sua conta na WiseHub foi atualizada com sucessoo !!");
    ...
    public ResponseEntity<ObjectNode> deleteUser(Long id) {
        try {
            ...
            mailService.enviarEmailCriacaoUsuario(usuario.getEmail(), "Exclusão de conta na wiseHub", "Olá Hooper, "+usuario.getNome()+" É triste saber que um Hooper como você se aposentou sua conta da WiseHub, espero que nas próximas seasons nós possamos nos ver novamente !!");
